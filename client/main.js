import { protocol as protocolDictionary } from "./protocol/protocol.js";
(function () {
    ...
    changelogTime: (function () {
      ...
      let seconds = Math.floor((duration / 1000) % 60),
        ...
        dateString =
          ...
          (minutes == 0
            ? ""
            : minutes + " minute" + (minutes == 1 ? "" : "s") + " "); // + (seconds == 0 ? "" : (minutes == 0 ? "" : " ") + seconds + " seconds");
      return dateString;
    })(),
    guiCornerRadius: 5,
    guiAlpha: 0.65,
  ...
  let player = {
    reset: function () {
      player = {
        ...
        spawn: function () {
          if (
            (config.gameState == config.gameStates.menu || player.dead) &&
            config.spawned == false
          )
            (elements.name.disabled = true),
              (config.spawned = true),
              ws.send(protocol.encode("spawn", elements.name.value)),
              localStorage.setItem("name", elements.name.value);
        },
        autoFire: false,
        kills: 0,
        deathMessage: "",
  ...
  util.fullScreenCanvas = function () {
    ...
    let ratio = Math.min(
      (config.screenHeight / Math.max(1080, config.screenHeight) +
        config.screenWidth / Math.min(1920, config.screenWidth)) /
        2,
      1
    );
  ...
  util.drawEntity = function (
    ...
    if (entityTypes[entity.class].shell != undefined) {
      ...
      if (entity.barrelFlash > 0.05) {
        ctx.fillStyle = util.mixColors(
          colors[shell.color],
          "#ffffff",
          entity.barrelFlash
        );
        ctx.strokeStyle = util.mixColors(
          darkColors[shell.color],
          "#ffffff",
          entity.barrelFlash
        );
      }
      ...
      util.drawPolygon(
        ...
        size * shell.size,
        shell.shape,
        animations.tankSpin * shell.spinSpeed
      );
      ctx.closePath();
      ctx.fill();
      ctx.stroke();
    ...
    for (let i2 = 0; i2 < entityTypes[entity.class].barrelsLength; i2++) {
      ...
      if (barrelAnimation) {
        util.drawTrapezoid(
          ctx,
          x - (entity.barrels[i2] * size) / 8,
          y - (barrel.width * size) / 2 + barrel.offset * size,
          barrel.height * size,
          barrel.width * size,
          barrel.trapezoidWidth
        );
      } else
        util.drawTrapezoid(
          ctx,
    ...
    if (entityTypes[entity.class].bodyShape != 0) {
      util.drawPolygon(
        ...
        x,
        y,
        size + size / entityTypes[entity.class].bodyShape,
        entityTypes[entity.class].bodyShape,
        entity.facing + Math.PI / entityTypes[entity.class].bodyShape
      );
      ctx.closePath();
      ctx.fill();
      ctx.stroke();
    ...
    if (entity.id != player.id && entity.showName == true) {
      ...
      if (entity.score != 0)
        util.drawText(ctx, entity.score, x, y - size * 1.35, size / 1.35);
    }
    ...
    if (alpha != 1) {
      ...
      ctx.drawImage(
        ...
        pos.x - size * 5,
        pos.y - size * 5,
        room.canvas.width * (1 / drawRatio),
        room.canvas.height * (1 / drawRatio)
      );
      ctx.globalAlpha = 1;
    }
  };
  ...
  for (let i = 0; i < colors.length; i++) {
    // Automatically make a dark version of every color.
    darkColors.push(util.mixColors(colors[i], "#000000", 0.25));
    //darkColors.push("#525252");
  }
  for (let i = 0; i < colors.length; i++) {
    // Automatically make a light version of every color.
    lightColors.push(util.mixColors(colors[i], "#ffffff", 0.5));
  }
  for (let i = 0; i < guiColors.length; i++) {
    // Automatically make a dark version of every guicolor.
    darkGuiColors.push(util.mixColors(guiColors[i], "#000000", 0.1));
  }
  for (let i = 0; i < guiColors.length; i++) {
    // Automatically make a light version of every guicolor.
  ...
  document.body.onmousedown = function (e) {
    if (
      ...
      if (
        util.rectCollide(
          {
            x: config.spacing,
            y: config.screenHeight - config.spacing - 30,
            width: 50,
            height: 30,
          },
          {
    ...
    if (config.gameState == config.gameStates.account) {
      if (
        util.rectCollide(
          {
            x: config.screenWidth - 50 - config.spacing,
            y: config.screenHeight - config.spacing - 30,
            width: 50,
            height: 30,
          },
    ...
    if (config.gameState == config.gameStates.menu && config.accountsEnabled) {
      ...
      if (
        util.rectCollide(
          {
            ...
            y:
              config.screenHeight -
              config.spacing * config.screenRatio -
              height * config.screenRatio,
        ...
        config.gameState = config.loggedIn
          ? config.gameStates.account
          : config.gameStates.login;
    ...
    if (player.skillPoints != 0) {
      ...
      let x =
        config.screenWidth -
        255 * config.screenRatio -
        config.spacing * config.screenRatio;
      let y =
        config.screenHeight / 2 -
        (player.skillLength * 30 * config.screenRatio +
          10 * config.screenRatio) /
          2;
      ...
      for (let i = 0; i < player.skillLength; i++) {
        ...
        let dy =
          config.mouse.y -
          (y - i * 30 * config.screenRatio - 25 * config.screenRatio);
        let distance = Math.sqrt(dx * dx + dy * dy);
    ...
    for (let key in player.upgrades) {
      ...
      let x =
        (config.screenWidth -
          animations.upgradeWidth * config.screenRatio -
          config.spacing * config.screenRatio +
          20 * config.screenRatio +
          upgrade.x * 120 * config.screenRatio) *
          upgrade.animation -
        150 * config.screenRatio * (-upgrade.animation + 1);
      let y =
        config.screenHeight -
        180 * config.screenRatio -
        config.spacing * config.screenRatio +
        60 * config.screenRatio;
      if (
        util.rectCollide(
          {
            x: config.mouse.x,
      ...
      if (
        util.rectCollide(
          ...
          },
          {
            ...
            y:
              config.screenHeight / 2 -
              50 * config.screenRatio -
              animations.menu * 400 * config.screenRatio -
              (animations.menuSlide * config.screenHeight) / 1.5,
  ...
  canvas.ontouchmove = function (e) {
    ...
    if (
      config.mouse.x < (animations.skillHover == 1 ? 255 : 10) &&
      config.mouse.y >
        config.screenHeight -
          config.spacing * config.screenRatio -
          player.skillLength * 30 * config.screenRatio -
          15 * config.screenRatio
    ) {
      animations.skillHover = 1;
    } else animations.skillHover = 0;
    config.mouse.pressing = true;
  ...
  const loop = function () {
    ...
    player.camera.ratio =
      (config.screenWidth + config.screenHeight) / 3000 / player.camera.fov;
    //if (player.mobile) config.screenRatio *= 1.25;
    ctx.resetTransform();
    if (
      config.gameState == config.gameStates.ingame ||
      ...
      player.facing = Math.atan2(
        config.mouse.y -
          config.screenHeight / 2 -
          (player.position.y - player.camera.position.y) * player.camera.ratio,
        config.mouse.x -
          config.screenWidth / 2 -
          (player.position.x - player.camera.position.x) * player.camera.ratio
      );
      ...
      player.camera.position.x = util.lerp(
        player.camera.position.x,
        player.camera.position.rx,
        0.05
      );
      player.camera.position.y = util.lerp(
        player.camera.position.y,
        player.camera.position.ry,
        0.05
      );
      player.camera.fov = util.lerp(
        player.camera.fov,
        player.camera.rfov,
        0.025
      );
      ...
      if (player.input[0])
        player.position.vx = util.lerp(player.position.vx, -player.speed, 0.1);
      if (player.input[1])
        player.position.vx = util.lerp(player.position.vx, player.speed, 0.1);
      if (player.input[2])
        player.position.vy = util.lerp(player.position.vy, -player.speed, 0.1);
      if (player.input[3])
        player.position.vy = util.lerp(player.position.vy, player.speed, 0.1);
      ctx.fillStyle = colors[5];
      ctx.fillRect(0, 0, config.screenWidth, config.screenHeight);
      ctx.translate(
        config.screenWidth / 2 - player.camera.position.x * player.camera.ratio,
        config.screenHeight / 2 - player.camera.position.y * player.camera.ratio
      );
        ...
        case "2tdm":
          {
            ...
            ctx.fillRect(
              -(room.width / 2) + baseSize / 2,
              room.height / 2 - baseSize - baseSize / 2,
              baseSize,
              baseSize
            );
            // draw red base
            ...
            ctx.fillRect(
              room.width / 2 - baseSize - baseSize / 2,
              -(room.height / 2) + baseSize / 2,
              baseSize,
              baseSize
            );
          }
      ...
      let addSize = 100000;
      ctx.fillRect(
        Math.round((-(room.width / 2) - addSize / 2) / player.camera.ratio) *
          player.camera.ratio,
        Math.round((-(room.height / 2) - addSize / 2) / player.camera.ratio) *
          player.camera.ratio,
        room.width + addSize,
        room.height + addSize
      );
      ...
      for (let key in room.entities) {
        ...
        entity.barrelFlash = util.lerp(
          entity.barrelFlash,
          entity.rbarrelFlash,
          0.25
        );
        entity.shieldFlash = util.lerp(
          entity.shieldFlash,
          entity.rshieldFlash,
          0.1
        );
        ...
        util.drawEntity(
          ...
          entity.y,
          entity,
          changeFullColor ? changeColor : undefined,
          entity.size,
          7.5,
          true,
          entity.alpha
      ...
      if (!player.showingTank) {
        ...
        util.roundRect(
          ctx,
          config.spacing,
          config.screenHeight - 150 - config.spacing,
          400,
          150,
          5
        );
        ...
        util.drawText(
          ...
          "Score: " + player.score,
          config.spacing + 20,
          config.screenHeight - 120 - config.spacing,
          20,
          "left"
        );
        util.drawText(
          ...
          "Level " + player.level,
          config.spacing + 20,
          config.screenHeight - 70 - config.spacing,
          20,
          "left"
        );
        util.drawText(
          ...
          player.killsText,
          config.spacing + 20,
          config.screenHeight - 20 - config.spacing,
          20,
          "left"
        );
        util.drawEntity(
        ...
        ctx.globalAlpha = config.guiAlpha;
        ctx.fillStyle = colors[3];
        animations.killsMenu = util.lerp(
          animations.killsMenu,
          player.kills == 0 ? 0 : 1,
          0.1
        );
        util.roundRect(
          ctx,
          config.screenWidth - 400 - config.spacing,
          config.spacing * animations.killsMenu -
            30 * (-animations.killsMenu + 1),
          80,
          30,
          config.guiCornerRadius
        );
        ctx.fill();
        ctx.globalAlpha = 1;
        ctx.fillStyle = colors[2];
        util.drawText(
          ...
          config.screenWidth - 400 - config.spacing + 40,
          config.spacing * animations.killsMenu -
            30 * (-animations.killsMenu + 1) +
            20,
          18,
        ...
        ctx.fillStyle = colors[4];
        ctx.fillRect(
          config.screenWidth - 200 - config.spacing,
          config.spacing,
          200,
          200
        );
          ...
          case "2tdm":
            {
              ...
              ctx.fillStyle = colors[10];
              ctx.fillRect(
                config.screenWidth - 200 - config.spacing + baseSize / 2,
                200 - baseSize * 1.5 + config.spacing,
                baseSize,
                baseSize
              );
              // draw red base
        ...
        if (room.gm == "2dom") {
          ...
          ctx.fillRect(
            config.screenWidth -
              ...
              100 -
              (200 / 3 + baseSize / 2),
            200 + config.spacing - 200 + 100 - baseSize / 2,
            baseSize,
            baseSize
          );
          // right
          ...
          ctx.fillStyle = colors[room.dominationColors[2]];
          ctx.fillRect(
            config.screenWidth - 200 - config.spacing + 100 - baseSize / 2,
            200 + config.spacing - 200 + 100 + (200 / 3 - baseSize / 2),
            baseSize,
            baseSize
          );
          // up
        ...
        ctx.arc(
          config.screenWidth -
            200 -
            config.spacing +
            player.position.x / (room.rwidth / 200) +
            room.rwidth / (room.rwidth / 100),
          config.spacing +
            player.position.y / (room.rheight / 200) +
            room.rheight / (room.rheight / 100),
          3,
          0,
          2 * Math.PI
        );
        ...
        ctx.fillStyle = colors[2];
        // Draw skills
        animations.skill = util.lerp(
          animations.skill,
          config.gameState == config.gameStates.goingInGame ||
            config.gameState == config.gameStates.ingame
            ? animations.skillHover == 0
        ...
        if (true) {
          ...
          util.roundRect(
            ctx,
            config.screenWidth - 255 - config.spacing,
            y,
            255,
            player.skillLength * 30 + 10,
            config.guiCornerRadius
          );
          ctx.fill();
          ctx.fillStyle = colors[0];
          ...
          for (let i = 0; i < player.skillLength; i++) {
            ...
            ctx.moveTo(x + 20, y - i * 30 - 25);
            ctx.lineTo(
              x + 20 + 215 * (skill.count / skill.max),
              y - i * 30 - 25
            );
            ctx.closePath();
            ctx.stroke();
            if (player.skillPoints == 0 && !skill.count >= skill.max)
              ctx.fillStyle = colors[7];
            ...
            ctx.fillStyle = colors[3];
            if (
              distance < 12.5 * config.screenRatio &&
              !(skill.count >= skill.max) &&
              player.skillPoints != 0
            ) {
              if (document.body.style.cursor != "pointer")
                document.body.style.cursor = "pointer";
            ...
            ctx.globalAlpha = 1;
            ctx.fillStyle = colors[0];
            if (skill.count >= skill.max || player.skillPoints == 0)
              ctx.fillStyle = colors[7];
        ...
        for (let i = 0; i < config.messages.length; i++) {
          ...
          util.roundRect(
            ctx,
            config.screenWidth / 2 - message.width / 2 - 10,
            config.spacing + i * 50,
            message.width + 20,
            40,
            10
        ...
        animations.upgradeWidth = util.lerp(
          animations.upgradeWidth,
          player.upgradeCount * 120 + 20,
          0.1
        );
        ...
        util.roundRect(
          ...
          config.screenWidth -
            animations.upgradeWidth -
            config.spacing +
            (-animations.upgrade * 300 + 300),
          config.screenHeight - 180 - config.spacing,
          animations.upgradeWidth,
          180,
          config.guiCornerRadius
        );
        ...
        util.drawText(
          ...
          config.screenWidth -
            ...
            (-animations.upgrade * 300 + 300) -
            20,
          config.screenHeight - 180 - config.spacing + 35,
          28,
          "right"
        );
        ...
        for (let key in player.upgrades) {
          ...
          util.drawUpgrade(
            ...
            (config.screenWidth -
              ...
              upgrade.x * 120) *
              upgrade.animation -
              150 * (-upgrade.animation + 1),
            config.screenHeight - 180 - config.spacing + 60,
            upgrade.color
          );
        }
        // draw leaderboard
        animations.lbWidth = util.lerp(
          animations.lbWidth,
          50 + room.lbLength * 35,
          0.1
        );
        const leaderboardWidthWithTankImages =
          config.leaderboardWidth + config.spacing + 5 + 5; // Additional space for tank images
        util.roundRect(
          ctx,
          config.spacing,
          config.spacing,
          leaderboardWidthWithTankImages,
          animations.lbWidth,
          config.guiCornerRadius
        );
        ctx.fillStyle = colors[config.guiColor];
        ...
        for (let i = 0; i < room.lbLength; i++) {
          ...
          ctx.beginPath();
          ctx.moveTo(
            config.leaderboardWidth / 2 - 110 + config.spacing,
            40 + i * 35 + config.spacing
          );
          ctx.lineTo(
            config.leaderboardWidth / 2 + 110 + config.spacing,
            40 + i * 35 + config.spacing
          ...
          ctx.lineTo(
            config.leaderboardWidth / 2 +
              110 * (tank.percentage * 2 - 1) +
              config.spacing,
            40 + i * 35 + config.spacing
          );
          ctx.closePath();
          ...
          util.drawEntity(
            ctx,
            config.leaderboardWidth + config.spacing + 10,
            40 + i * 35 + config.spacing,
            {
              class: tank.class,
              showHealth: false,
          ...
          util.drawText(
            ctx,
            tank.name + " - " + tank.score,
            config.leaderboardWidth / 2 + config.spacing,
            45 + i * 35 + config.spacing,
            20
          );
        }
      }
      if (player.showingTank) {
        ...
        util.drawText(
          ...
          config.screenHeight / 8 - 70,
          20
        );
        util.drawUpgrade(
          { tank: player.tank.id, size: 1, detectClick: false },
          config.screenWidth / 8 - 50,
          config.screenHeight / 8 - 50,
          Math.floor(Math.random() * guiColors.length)
        );
      ...
      config.gameState == config.gameStates.connecting ||
      config.gameState == config.gameStates.goingToMenu ||
      config.gameState == config.gameStates.goingToLoadingET
    ) {
      if (elements.name.style.display == "block")
        elements.name.style.display = "none";
      if (elements.loginInput.style.display == "block")
        elements.loginInput.style.display = "none";
      ...
      if (player.connectingWarn) {
        animations.connectingWarn = util.lerp(
      ...
      util.drawText(
        ...
        config.screenWidth / 2,
        config.screenHeight / 2 - 300 + animations.loadingET * 200,
        30,
        "center"
      );
      ctx.globalAlpha = 1;
      ...
      util.drawText(
        ...
        "Connecting...",
        config.screenWidth / 2,
        config.screenHeight / 2 - animations.connecting * 200,
        75,
        "center"
      );
      ...
      util.drawText(
        ctx,
        "Try switching to http or https if you cannot connect",
        config.screenWidth / 2,
        config.screenHeight / 2 + animations.connectingWarn * 50,
        20,
        "center"
      );
      // animations.connectingWarn
    ...
    if (
      ...
      if (config.gameState == config.gameStates.goingInGame) {
        ...
        if (animations.menuSlide > 0.99 && animations.menuBGAlpha < 0.01)
          (config.gameState = config.gameStates.ingame),
            (config.sendInput = true);
      } else
        animations.menuBGAlpha = util.lerp(animations.menuBGAlpha, 1, 0.05);
      if (elements.name.style.display == "none")
      ...
      util.drawText(
        ctx,
        "Changelog",
        config.screenWidth - config.spacing - 25,
        config.spacing + 10,
        20,
        "right"
      );
      util.drawText(
        ctx,
        config.changelogTime == ""
          ? "Updated just now"
          : "Updated " + config.changelogTime + "ago",
        config.screenWidth - config.spacing - 25,
        config.spacing + 40,
        18,
        "right"
      ...
      for (let i = 0; i < window.changelog.length; i++) {
        if (window.changelog[i] == "Added some aggressive bots")
          ctx.fillStyle = "#ff0000";
        util.drawText(
          ctx,
          window.changelog[i] + " -",
          config.screenWidth - config.spacing - 25,
          config.spacing + 70 + i * 30,
          12.5,
          "right"
        );
        ctx.fillStyle = colors[0];
      ...
      if (player.mobile)
        util.drawText(
          ctx,
          "NOTE: Mobile version of PolyTonk is not finished yet.",
          config.screenWidth / 2,
          config.screenHeight / 2 -
            180 -
            animations.menu * 400 -
      ...
      util.roundRect(
        ctx,
        animations.servers * 1000 - 1005,
        config.screenHeight / 2 - 180,
        700,
        125 + window.servers.length * 100,
        20
      );
      ctx.fill();
      ...
      for (let i = 0; i < window.servers.length; i++) {
        util.drawText(
          ctx,
          window.servers[i].name + (config.selectedServer == i ? " <" : ""),
          animations.servers * 1000 - 1005,
          config.screenHeight / 2 + i * 100,
          50,
          "left"
        );
      }
      ...
      elements.name.style.top =
        ...
        "px";
      elements.name.style.left =
        ((config.screenWidth / 2 - 150) * config.screenRatio) /
          window.devicePixelRatio +
        "px";
      elements.name.style.width =
        (300 * config.screenRatio) / window.devicePixelRatio + "px";
      elements.name.style.height =
        (35 * config.screenRatio) / window.devicePixelRatio + "px";
      elements.name.style.fontSize =
        (35 * config.screenRatio) / window.devicePixelRatio + "px";
      config.screenWidth *= config.screenRatio;
      config.screenHeight *= config.screenRatio;
      ctx.resetTransform();
    ...
    if (player.dead || player.dead == 2) {
      ...
      const backgroundY =
        (config.screenHeight - textBackgroundHeight) / 2 +
        (400 - animations.deathScreen * 400); // Adjust based on animation progress
      ...
      ctx
        .roundRect(
          ...
          textBackgroundWidth + borderThickness * 2,
          textBackgroundHeight + borderThickness * 2,
          cornerRadius
        )
        .fill();

      // Draw grey background
      ctx.fillStyle = "rgba(82, 82, 82, 0.8)"; // Adjust background color and alpha value as needed
      ctx
        .roundRect(
          backgroundX,
          backgroundY,
      ...
      util.drawText(
        ...
        player.deathMessage,
        config.screenWidth / 2,
        config.screenHeight / 2 - 50,
        25,
        "center"
      );
      util.drawText(
        ctx,
        "Died to a " + entityTypes[player.killerType].name,
        config.screenWidth / 2,
      ...
      util.drawText(
        ctx,
        "Level " + player.level + " " + player.tank.name,
        config.screenWidth / 2 - 150,
        config.screenHeight / 2,
        25,
        "left"
    ...
    if (config.disconnected) {
      ...
      util.drawText(
        ...
        "Disconnected",
        config.screenWidth / 2,
        config.screenHeight / 2 - 400 - -animations.disconnected * 400,
        60,
        "center"
      );
      util.drawText(
        ...
        "Reconnecting...",
        config.screenWidth / 2,
        config.screenHeight / 2 + 50 - 400 - -animations.disconnected * 400,
        30,
        "center"
      );
      ctx.globalAlpha = 1;
    ...
    if (config.gameState == config.gameStates.login) {
      ...
      util.drawText(
        ...
        "PolyTonk",
        config.screenWidth / 2,
        config.screenHeight / 2 + animations.login2 * 200 - 150,
        75,
        "center"
      );
      ...
      if (
        ...
        animations.loginButtonRegister = util.lerp(
          animations.loginButtonRegister,
          1,
          0.1
        );
      ...
      util.drawText(
        ctx,
        "Login",
        config.screenWidth / 2 - 60,
        config.screenHeight / 2 + 97.5,
        15 + animations.loginButtonRegister * 2,
        "center"
      );
      // Register button
      hovering = false;
      if (
        ...
        animations.loginButtonLogin = util.lerp(
          animations.loginButtonLogin,
          1,
          0.1
        );
        dontRevertPointer = true;
      ...
      util.drawText(
        ...
        config.screenWidth / 2 + 60,
        config.screenHeight / 2 + 97.5,
        15 + animations.loginButtonLogin * 2,
        "center"
      );
      ...
      if (
        ...
        hovering = true;
      if (hovering) {
        animations.loginButtonBack = util.lerp(
          animations.loginButtonBack,
          1,
          0.1
        );
      ...
      util.roundRect(
        ctx,
        config.spacing,
        config.screenHeight - config.spacing - 30,
        50,
        30,
        10
      );
      ...
      util.roundRect(
        ctx,
        config.spacing,
        config.screenHeight - config.spacing - 15,
        50,
        15,
        {
          bl: config.guiCornerRadius,
      ...
      util.drawText(
        ...
        "Back",
        config.spacing + 25,
        config.screenHeight - config.spacing - 10,
        15 + animations.loginButtonBack * 2,
        "center"
      );
      ctx.fillStyle = colors[0];
      ctx.globalAlpha = animations.login;
      ctx.fillRect(0, 0, config.screenWidth, config.screenHeight);
      if (!dontRevertPointer)
        if (document.body.style.cursor == "pointer")
          document.body.style.cursor = "default";
    }
    if (config.gameState == config.gameStates.account) {
      ...
      util.drawText(
        ctx,
        player.account.name,
        config.screenWidth / 2 - 200 + 70,
        config.screenHeight / 2 - 100 + 40,
        30,
        "left"
      );
      util.drawEntity(
        ctx,
        config.screenWidth / 2 - 200 + 35,
        config.screenHeight / 2 - 100 + 30,
        {
      ...
      util.drawText(
        ctx,
        "Total score: " + player.account.score,
        config.screenWidth / 2 - 200 + config.inSpacing,
        config.screenHeight / 2 - 100 + 50 + 40,
        20,
        "left"
      );
      util.drawText(
        ctx,
        "Account level: " + player.account.level,
        config.screenWidth / 2 - 200 + config.inSpacing,
        config.screenHeight / 2 - 100 + 50 + 40 * 2,
        20,
        "left"
      );
      util.drawText(
        ...
        config.screenWidth / 2 - 200 + config.inSpacing,
        config.screenHeight / 2 - 100 + 50 + 40 * 3,
        20,
        "left"
      );
      util.drawText(
        ctx,
        "Deaths: " + player.account.deaths,
        config.screenWidth / 2 - 200 + config.inSpacing,
        config.screenHeight / 2 - 100 + 50 + 40 * 4,
        20,
        "left"
      );
      util.drawText(
        ctx,
        "Created on " + player.account.creationDate,
        config.screenWidth / 2 - 200 + config.inSpacing,
        config.screenHeight / 2 - 100 + 50 + 40 * 6,
        15,
        "left"
      );
      // Back button
      ...
      util.roundRect(
        ctx,
        config.screenWidth - 50 - config.spacing,
        config.screenHeight - config.spacing - 30,
        50,
        30,
        10
      ...
      util.drawText(
        ctx,
        "Logout",
        config.screenWidth - 50 - config.spacing + 25,
        config.screenHeight - config.spacing - 10,
        15 + animations.logoutButton * 2,
        "center"
      );
      // Back button
      ctx.fillStyle = colors[10];
    ...
    if (config.gameState == config.gameStates.ready) {
      ...
      util.drawText(
        ctx,
        "Welcome beta tester! Do you think this update should be released? Tell in the discord!",
        config.screenWidth / 2,
        config.screenHeight / 2,
        20,
        "center"
  ...
  function connect(server) {
    ...
    ws.onmessage = function (message) {
      ...
      switch (type) {
        ...
        case "goingInGame":
          console.log(
            "%cWelcome to PolyTonk",
            "font-style:bold;color:#ffb347;font-size:50px;"
          );
          config.gameState = config.gameStates.goingInGame;
          player.resetSkill();
          player.skillPoints = 0;
            ...
            if (!(key in room.entities)) {
              ...
              for (let key2 in room.entities) {
                ...
                if (key2 == key) {
                  ...
                  if (
                    room.entities[key2].rbarrelsLength !=
                    message[key].barrelsLength
                  ) {
                    room.entities[key2].rbarrelsLength =
                      message[key].barrelsLength;
                    room.entities[key2].barrelsLength =
                      message[key].barrelsLength;
                  ...
                  if (room.entities[key2].rhealth > message[key].health)
                    room.entities[key2].damageAnimation = 1;
        ...
        case "account":
          if (message[0]) {
            // If the account packet says we are in the account
            config.loggedIn = true;
          } else {
            // else, grab the account data server gave you
            player.account.score = message[1];
            player.account.level = message[2];
            player.account.color = message[3];
            player.account.name = message[4];
            player.account.creationDate = new Date(
              parseInt(message[5])
        ...
        case "death":
          ...
          if (player.score > 1000000) {
            player.deathMessage =
              "Oh wow, you should probably post this somewhere";
          }
          ...
          if (player.score < 22275 && player.score > 20000) {
            player.deathMessage =
              "You almost reached level 45, what a bad luck";
          }
          if (player.score == 0) {
