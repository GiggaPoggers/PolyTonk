...
(function() {
  ...
  document.getElementById("btnUp").addEventListener("mouseup", function() {
      player.input[2] = false;
  });
  document.getElementById("btnLeft").addEventListener("mouseup", function() {
      player.input[0] = false;
  });
  document.getElementById("btnRight").addEventListener("mouseup", function() {
      player.input[1] = false;
  });
  document.getElementById("btnDown").addEventListener("mouseup", function() {
      player.input[3] = false;
  });
  ...
  const loop = function() {
      ...
      player.camera.ratio = ((config.screenWidth + config.screenHeight) / 3000) / player.camera.fov;
        ...
        if (config.gameState == config.gameStates.menu || config.gameState == config.gameStates.goingInGame || config.gameState == config.gameStates.goingToMenuFromDeath) {
          ...
          //menu input
            ctx.fillStyle = colors[8];
            util.roundRect(ctx, config.screenWidth / 2 - 150, (config.screenHeight / 2 - 48 - animations.menu * 400) - animations.menuSlide * config.screenHeight / 1.5, 300, 35, 10);
            ctx.fill();
            ctx.fillStyle = colors[2];
            util.roundRect(ctx, config.screenWidth / 2 - 150, (config.screenHeight / 2 - 50 - animations.menu * 400) - animations.menuSlide * config.screenHeight / 1.5, 300, 35, 10);
            ctx.fill();
            ...
            elements.name.style.fontSize = (25 * config.screenRatio) / window.devicePixelRatio + "px"
            config.screenWidth *= config.screenRatio;
            config.screenHeight *= config.screenRatio;
            ctx.resetTransform();
