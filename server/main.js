    ...
    case 0:
      {
        ...
        switch (b) {
          case 0:
            {
              // Orange, down left corner
              if (a == 0) {
                return util.lerp(
                  -(room.width / 2) + baseSize / 2,
                  -(room.width / 2) + baseSize / 2 + baseSize,
                  0.5
                );
              } else
                return util.lerp(
                  room.height / 2 - baseSize - baseSize / 2,
                  room.height / 2 - baseSize - baseSize / 2 + baseSize,
                  0.5
                );
            }
            break;
          case 1:
            {
              ...
              if (a == 0) {
                return util.lerp(
                  ...
                  room.width / 2 - baseSize - baseSize / 2 + baseSize,
                  0.5
                );
              } else
                return util.lerp(
                  -(room.height / 2) + baseSize / 2,
                  -(room.height / 2) + baseSize / 2 + baseSize,
                  0.5
                );
            }
            break;
...
server.entityTypesPacket = protocol.encode(
  "entityTypes",
  JSON.stringify(entityConfig.entityTypeMockups)
);
...
const room = {
  ...
  shapeAmount: 0,
  maxShapes: config.maxShapesDependsOnMapSize
    ? (config.width + config.height) / 250
    : config.maxShapes,
  gm: config.gm,
  teams: (function () {
...
class Barrel {
  ...
  shoot() {
    ...
    this.reloadTime =
      (this.reload * (server.fps / 2)) /
      Math.sqrt(Math.sqrt(room.entities[this.owner].skill.rld + 1));
    let x =
      entity.x +
      Math.cos(entity.facing + this.angle) * entity.size * this.height +
      Math.cos(entity.facing + this.angle + this.offset) * entity.size;
    let y =
      entity.y +
      Math.sin(entity.facing + this.angle) * entity.size * this.height +
      Math.sin(entity.facing + this.angle + this.offset) * entity.size;
    let bullet = new Entity(
      x,
      y,
      "",
      idGenerator.generateId(),
      (entity.size * this.width) / 2,
      "bullet"
    );
    ...
    bullet.vx =
      Math.cos(entity.facing + this.angle + spraySize) *
      (20 + room.entities[this.owner].skill.bls * 3) *
      this.spdFactor;
    bullet.vy =
      Math.sin(entity.facing + this.angle + spraySize) *
      (20 + room.entities[this.owner].skill.bls * 3) *
      this.spdFactor;
    ...
    bullet.health =
      (1 + room.entities[this.owner].skill.pen / 2) * this.penFactor;
    ...
    bullet.damage =
      (1 +
        room.entities[this.owner].skill.dmg +
        (room.entities[this.owner].skill.pen * this.penFactor + 1) / 10) *
      this.damageFactor;
  }
  loop() {
...
class Entity {
  constructor(
    ...
    this.getPointsOn = [
      ...
      22, 23, 24, 25, 26, 28, 29, 30, 32, 33, 35, 37, 39, 41, 43, 45, 46, 47,
      48, 49, 50,
    ];
    this.skill = {
      rld: 0, // Reload
          ...
          case 0:
            {
              ...
              switch (this.team) {
                case 0:
                  {
                    // Orange
                    this.x = util.getRandomFromRange(
                      -(room.width / 2) + baseSize / 2,
                      -(room.width / 2) + baseSize / 2 + baseSize
                    ); // left
                    this.y = util.getRandomFromRange(
                      room.height / 2 - baseSize - baseSize / 2,
                      room.height / 2 - baseSize - baseSize / 2 + baseSize
                    ); // bottom
                  }
  ...
  define(id) {
    ...
    for (let i = 0; i < entityConfig.entityTypes[id].barrels.length; i++) {
      ...
      this.barrels.push(
        new Barrel(
          ...
          barrel.offset,
          barrel.recoil,
          barrel.bullet == undefined ? 1 : barrel.bullet.damageFactor,
          barrel.bullet == undefined ? 1 : barrel.bullet.rangeFactor,
          barrel.bullet == undefined ? 1 : barrel.bullet.penFactor,
          barrel.bullet == undefined ? 1 : barrel.bullet.spdFactor,
          barrel.bullet == undefined ? 1 : barrel.bullet.sprFactor,
          barrel.canShoot
        )
      );
    }
    ...
    if (this.connection != null)
      this.connection.send(protocol.encode("upgradeReset"));
    this.upgradesToGet =
      entityConfig.upgradeTree[entityConfig.entityTypes[this.class].name];
    ...
    if (!this.ranOnDeath) {
      ...
      if (this.connection != null) {
        if (this.connection.loggedIn) {
          if (addKill)
            accounts[this.connection.accountID].deaths = parseInt(
              accounts[this.connection.accountID].deaths + 1
            );
          accounts[this.connection.accountID].kills =
            parseInt(accounts[this.connection.accountID].kills) + this.kills;
          accounts[this.connection.accountID].score =
            parseInt(accounts[this.connection.accountID].score) + this.score;
          accounts[this.connection.accountID].level =
            Math.sqrt(accounts[this.connection.accountID].score / 20000) *
            Math.max(1, accounts[this.connection.accountID].score / 500000);
        }
      }
    }
  }
  ...
  loop() {
    ...
    if (this.hasAI) {
      ...
      for (let key in room.entities) {
        if (
          key != this.id &&
          (this.AISettings.ignoresOwnTeam
            ? room.entities[key].team != this.team
            : true) &&
          (this.AISettings.attacksDefaultTeam
            ? true
            : room.entities[key].team != -1) &&
          (this.AISettings.onlyAttacksTanks
            ? room.entities[key].type == "tank"
            : true) &&
          room.entities[key].type != "bullet"
        ) {
          entities.push({
            id: key,
      ...
      if (entity != undefined) {
        let pos = {
          x:
            entity.x + entity.vx * 8 - this.x + Math.cos(this.orbitAngle) * 800,
          y:
            entity.y + entity.vy * 8 - this.y + Math.sin(this.orbitAngle) * 800,
        };
        if (this.AISettings.randomAim ? Math.random() > 0.9 : true) {
          let facing = Math.atan2(
            entity.y + entity.vy * 8 - this.y,
            entity.x + entity.vx * 8 - this.x
          );
          this.facing = util.lerpAngle(
            this.facing,
            facing,
            this.AISettings.facingLerp
          );
    ...
    if (this.type == "bDrone") {
      ...
      if (!this.goToBase) {
        if (entities.length == 0) {
          ...
          entities = [
            {
              x:
                util.getBasePos(0, this.team) + Math.cos(this.orbitAngle) * 300,
              y:
                util.getBasePos(1, this.team) + Math.sin(this.orbitAngle) * 300,
            },
          ];
        }
      }
      ...
      this.facing = util.lerpAngle(
        this.facing,
        Math.atan2(entity.y - this.y, entity.x - this.x) + Math.PI,
        0.45
      );
      this.vx = util.lerp(
        this.vx,
        Math.cos(this.facing + Math.PI) * this.speed,
        0.1
      );
      this.vy = util.lerp(
        this.vy,
        Math.sin(this.facing + Math.PI) * this.speed,
        0.1
      );
    }
    ...
    if (this.health < this.previousFrame.health)
      this.healTime = Date.now() + 20000; // Wait for 10 seconds to heal
    if (this.health != this.previousFrame.health) {
    ...
    if (
      (this.skill.hlt != this.previousFrame.skill.hlt &&
        this.type == "tank" &&
        this.previousFrame.class != this.class) ||
      ((this.type == "tank" ? this.level != this.previousFrame.level : false) &&
        this.changeMaxHealth)
    ) {
      //let previousHealth = this.health / this.maxHealth;
      this.maxHealth =
        (this.healthBasedOnLevel ? util.getHealthFromLevel(this.level) : 10) +
        (this.skill.hlt * this.skill.hlt) / 50;
    ...
    if (
      this.skill.bdmg != this.previousFrame.skill.bdmg &&
      this.type == "tank"
    ) {
    ...
    if (this.type == "tank") {
      if (
        this.level != this.previousFrame.level ||
        this.skill.spd != this.previousFrame.skill.spd ||
        this.upgradesToGet != this.previousFrame.upgradesToGet
      ) {
        if (this.upgradesToGet.tier1.length != 0) {
          if (
            this.level >= config.tierConfig.tier1 &&
            !this.gottenUpgrades.tier1
          ) {
            ...
            for (let i = 0; i < this.upgradesToGet.tier1.length; i++) {
              ...
              if (this.connection != null)
                this.connection.send(
                  protocol.encode("upgrade", this.upgradesToGet.tier1[i])
                );
            }
        ...
        if (this.upgradesToGet.tier2.length != 0) {
          if (
            this.level >= config.tierConfig.tier2 &&
            !this.gottenUpgrades.tier2
          ) {
            ...
            for (let i = 0; i < this.upgradesToGet.tier2.length; i++) {
              ...
              if (this.connection != null)
                this.connection.send(
                  protocol.encode("upgrade", this.upgradesToGet.tier2[i])
                );
            }
            if (this.AISettings.upgrade) {
              this.define(
        ...
        if (this.upgradesToGet.tier3.length != 0) {
          if (
            this.level >= config.tierConfig.tier3 &&
            !this.gottenUpgrades.tier3
          ) {
            ...
            for (let i = 0; i < this.upgradesToGet.tier3.length; i++) {
              ...
              if (this.connection != null)
                this.connection.send(
                  protocol.encode("upgrade", this.upgradesToGet.tier3[i])
                );
            }
            if (this.AISettings.upgrade) {
              this.define(
        ...
        this.fov = util.getFOV(this.size) * this.fieldFactor;
        this.speed =
          util.getSpeedFromLevel(this.level) *
          (this.skill.spd / 10 + 1) *
          this.factor.speed;
        ...
        for (let i2 = 0; i2 < this.getPointsOn.length; i2++) {
          if (this.getPointsOn.includes(this.level - i)) {
            ...
            if (this.AISettings.upgradeSkill) {
              ...
              randomChoose =
                randomChoose[Math.floor(Math.random() * randomChoose.length)];
              this.skill[randomChoose]++;
            }
            if (this.connection != null)
              this.connection.send(protocol.encode("skill", 1));
          }
        }
      }
    }
    ...
    if (!this.canBypassBorder) {
      if (this.x > room.width - room.width / 2 + config.mapBorderSize)
        this.x = room.width - room.width / 2 + config.mapBorderSize;
      if (this.y > room.height - room.height / 2 + config.mapBorderSize)
        this.y = room.height - room.height / 2 + config.mapBorderSize;
      if (this.x < -room.width / 2 - config.mapBorderSize)
        this.x = -room.width / 2 - config.mapBorderSize;
      if (this.y < -room.height / 2 - config.mapBorderSize)
        this.y = -room.height / 2 - config.mapBorderSize;
    }
    if (this.dying) {
      this.collides = false;
      this.damages = false;
      ...
      if (this.dieTime < 0) {
        ...
        if (this.connection != null)
          (this.connection.spawned = false),
            (this.connection.dead = true),
            this.connection.send(
              protocol.encode(
                "death",
                room.entities[this.lastHurtedEnemy] == undefined
                  ? 0
                  : room.entities[room.entities[this.lastHurtedEnemy].owner] ==
                    undefined
                  ? room.entities[this.lastHurtedEnemy].class
                  : room.entities[room.entities[this.lastHurtedEnemy].owner]
                      .class
              )
            ),
...
app.ws("/server", function (ws, req) {
  ...
  if (
    ipBans.includes(crypto.createHash("sha256").update(ws.ip).digest("hex"))
  ) {
    ws.terminate();
    return;
  }
    ...
    switch (type) {
      ...
      case "error":
        {
          console.log(
            prefix +
              "WARNING: A packet has encountered a error while getting decoded."
          );
          ws.ban();
          return;
        }
      ...
      case "spawn":
        {
          if (!ws.spawned) {
            if (message[0].length > 24) {
              console.log(
                prefix +
                  "INFO: A client connected with a name longer than 24 letters."
              );
              ws.close();
              return;
            }
            ws.spawned = true;
            console.log(
              prefix + "INFO: A client is spawning... Name: " + message[0]
            );
            ws.send(protocol.encode("goingInGame"));
            let player = new Entity(
              Math.random() * room.width - room.width / 2,
              Math.random() * room.height - room.height / 2,
              message[0],
              ws.id,
              50,
              "tank",
            ...
            console.log(
              prefix +
                "WARNING: A client is trying to spawn while already spawned. Name: " +
                message[0]
            );
            ws.close();
            return;
            ...
            case "eval":
              if (ws.terminalAccessLevel == 2) {
                try {
                  ...
                  ws.send(
                    protocol.encode("terminalOutput", "Output: " + output)
                  );
                } catch (e) {
                ...
                ws.send(
                  protocol.encode(
                    "terminalOutput",
                    "Permission error: Your terminalAccessLevel must be atleast 2 to use this command"
                  )
                );
                return;
              }
                ...
                if (ws.terminalAccessLevel == 2) {
                  ...
                  if (stuff.length != 3) {
                    ws.send(
                      protocol.encode(
                        "terminalOutput",
                        'Please use the command correctly. Usage: "setUserPermission user_name permission"'
                      )
                    );
                    return;
                  }
                  for (let key in accounts) {
                    if (stuff[1] == accounts[key].name) {
                      ...
                      ws.send(
                        protocol.encode(
                          "terminalOutput",
                          "Succesfully set the permission of account " +
                            accounts[key].name +
                            " to " +
                            stuff[2]
                        )
            ...
            case "playerList":
              if (ws.terminalAccessLevel >= 1) {
                ...
                ws.send(
                  protocol.encode(
                    "terminalOutput",
                    "Permission error: Your terminalAccessLevel must be atleast 1 to use this command"
                  )
                );
                return;
              }
            ...
            case "ban":
              if (ws.terminalAccessLevel >= 2) {
                ...
                ws.send(
                  protocol.encode(
                    "terminalOutput",
                    "Permission error: Your terminalAccessLevel must be atleast 22 to use this command"
                  )
                );
                return;
              }
              ...
              if (ws.loggedIn)
                ws.send(
                  protocol.encode(
                    "account",
                    0,
                    Math.floor(accounts[ws.accountID].score),
                    Math.floor(accounts[ws.accountID].level),
                    accounts[ws.accountID].color,
                    accounts[ws.accountID].name,
                    accounts[ws.accountID].creationDate,
                    accounts[ws.accountID].tank,
                    accounts[ws.accountID].kills,
                    accounts[ws.accountID].deaths
                  )
                );
              break;
          }
      ...
      case "levelUp":
        {
          if (ws.spawned) {
            if (config.publicCheatingLevel >= 1) {
              if (
                room.entities[ws.id].score <=
                util.getScoreFromLevel(room.entities[ws.id].maxLevel)
              ) {
                room.entities[ws.id].level++;
                room.entities[ws.id].score = util.getScoreFromLevel(
                  room.entities[ws.id].level
                );
              }
            }
      ...
      case "upgrade":
        {
          if (ws.spawned) {
            if (message[0] > room.entities[ws.id].upgrades.length - 1) {
              ...
              room.entities[ws.id].define(
                room.entities[ws.id].upgrades[message[0]]
              );
          }
        }
        break;
      ...
      case "login": {
        if (message[0] == 1) {
          ...
          if (ws.loggedIn) {
            ws.send(
              protocol.encode("terminalOutput", "You are already logged in!")
            );
            return;
          }
          ...
          if (message[1] == "") {
            ws.send(
              protocol.encode("terminalOutput", "Your name cannot be empty")
            );
            return;
          }
          if (message[2] == "") {
            ws.send(
              protocol.encode("terminalOutput", "Your password cannot be empty")
            );
            return;
          }
          if (message[2].length <= 4) {
            ws.send(
              protocol.encode(
                "terminalOutput",
                "Your password needs to be atleast 5 letters long"
              )
            );
            return;
          }
          ...
          if (result == 0) {
            ws.send(
              protocol.encode("terminalOutput", "Sorry, name already taken.")
            );
          } else {
            if (message[1].includes(" ")) {
              ws.send(
                protocol.encode(
                  "terminalOutput",
                  "Please prevent your name from having a space"
                )
              );
              return;
            }
            ws.send(
              protocol.encode(
                "terminalOutput",
                "You are now successfully registered. To login, use the password and username you used."
              )
            );
          }
        } else {
          ...
          if (result == false) {
            ws.send(
              protocol.encode("terminalOutput", "Wrong name or password")
            );
          } else {
            ws.send(
              protocol.encode(
                "terminalOutput",
                "Succesfully logged in to the acc " +
                  accounts[result[1]].name +
                  ", your access level: " +
                  accounts[result[1]].terminalAccessLevel
              )
            );
...
for (let i = 0; i < baseCount; i++) {
  for (let i2 = 0; i2 < 4; i2++) {
    let baseSize = room.width / room.baseSize;
    let baseDrone = new Entity(
      util.getBasePos(0, i) + util.getRandomFromRange(-100, 100),
      util.getBasePos(1, i) + util.getRandomFromRange(-100, 100),
      "",
      idGenerator.generateId(),
      20,
      "drone"
...
function createShape(
  ...
  superAlpha = alpha ? Math.random() > 0.9 : false,
  spawnPosition = [
    Math.random() * room.width - room.width / 2,
    Math.random() * room.height - room.height / 2,
  ]
  ...
  let shape = new Entity(
    ...
    "",
    idGenerator.generateId(),
    (40 + choose * 15) * (alpha ? 2 : 1) * (superAlpha ? 2 : 1),
    "food",
    null,
    true
  );
  ...
  shape.maxHealth =
    (choose + 1) * (choose + 1) * 5 * (alpha ? 250 : 1) * (superAlpha ? 5 : 1);
  shape.health = shape.maxHealth;
  shape.value =
    (choose + 1) * (choose + 1) * 25 * (alpha ? 5 : 1) * (superAlpha ? 10 : 1);
  return shape;
}

function createShapeTree() {
  let mainShape = createShape(undefined, true, true, [
    Math.random() * room.width - room.width / 2,
    Math.random() * room.height - room.height / 2,
  ]);
  let count = 1;
  ...
  for (let i2 = 0; i2 < 2; i2++) {
    ...
    let alphaShape = createShape(undefined, true, false, [
      mainShape.x + Math.cos(angle) * 500,
      mainShape.y + Math.sin(angle) * 500,
    ]);
    ...
    for (let i3 = 0; i3 < 5; i3++) {
      ...
      let smallShape = createShape(undefined, false, false, [
        alphaShape.x + Math.cos(angle) * 500,
        alphaShape.y + Math.sin(angle) * 500,
      ]);
...
function gameLoop() {
  ...
  if (room.shapeAmount < room.maxShapes) {
    ...
    if (alpha) {
      spawnPosition = [
        util.getRandomFromRange(-room.width / 10, room.width / 10),
        util.getRandomFromRange(-room.height / 10, room.height / 10),
      ];
    }
    let shape = new Entity(
      spawnPosition[0],
    ...
    shape.facing = Math.random() * (Math.PI * 2);
    shape.maxHealth =
      (choose + 1) * (choose + 1) * 5 * (alpha ? 50 : 1) * (superAlpha ? 5 : 1);
    shape.health = shape.maxHealth;
    shape.damage *= 0.1;
    shape.value =
      (choose + 1) *
  ...
  for (let key in room.entities) {
    ...
    for (let candidate in candidates) {
      ...
      /*room.entities[candidate].size / 2;
            entity.size / 2;*/
      if (
        distance < candidateEntity.size + entity.size &&
        candidateEntity.id != entity.id
      ) {
        let angle = Math.atan2(
          entity.y - candidateEntity.y,
          entity.x - candidateEntity.x
        );
        if (room.entities[candidateEntity.id] != undefined) {
          ...
          if (
            room.entities[candidateEntity.id].team != entity.team &&
            room.entities[candidateEntity.id].type == "bullet" &&
            entity.type == "bullet"
          )
            (damage = true), (collide = true);
          if (
            room.entities[candidateEntity.id].team == entity.team &&
            room.entities[candidateEntity.id].type == "food" &&
            entity.type == "food"
          )
            (damage = false), (collide = true);
          if (
            room.entities[candidateEntity.id].team == entity.team &&
            room.entities[candidateEntity.id].type == "bDrone" &&
            entity.type == "bDrone"
          )
            (damage = false), (collide = true);
          if (
            room.entities[candidateEntity.id].team == entity.team &&
            room.entities[candidateEntity.id].type == "tank" &&
            entity.type == "tank"
          )
            (damage = false), (collide = true);
          if (
            collide &&
            room.entities[candidateEntity.id].collides &&
            entity.collides
          ) {
            room.entities[candidateEntity.id].vx -=
              Math.cos(angle) / (5 / (entity.size / 50));
            room.entities[candidateEntity.id].vy -=
              Math.sin(angle) / (5 / (entity.size / 50));
            entity.vx +=
              Math.cos(angle) /
              (5 / (room.entities[candidateEntity.id].size / 50));
            entity.vy +=
              Math.sin(angle) /
              (5 / (room.entities[candidateEntity.id].size / 50));
          }
          if (
            damage &&
            room.entities[candidateEntity.id].damages &&
            entity.damages
          ) {
            ...
            if (room.entities[candidateEntity.id].team != entity.team) {
              entity.health -=
                room.entities[candidateEntity.id].damage /
                Math.sqrt(Math.sqrt(entity.damage));
              room.entities[candidateEntity.id].health -=
                entity.damage /
                Math.sqrt(Math.sqrt(room.entities[candidateEntity.id].damage));
            }
          }
        }
      }
  ...
  for (let key in room.clients) {
    ...
    if (room.clients[key].spawned) {
      if (room.clients[key].team != room.entities[room.clients[key].id].team)
        room.clients[key].team = room.entities[room.clients[key].id].team;
    ...
    let candidates = room.tree.retrieve({
      x:
        room.clients[key].camera.x -
        (room.clients[key].camera.fov * 1777) / 1.5,
      y:
        room.clients[key].camera.y -
        (room.clients[key].camera.fov * 1000) / 1.5,
    ...
    for (let candidate in candidates) {
      ...
      if (room.entities[candidateEntity.id] != undefined) {
        ...
        for (
          let i = 0;
          i < room.entities[candidateEntity.id].barrels.length;
          i++
        ) {
          barrels.push(
            room.entities[candidateEntity.id].barrels[i].barrelAnimation
          );
        }
        entities.push({
          x: room.entities[candidateEntity.id].x,
          y: room.entities[candidateEntity.id].y,
          name: room.entities[candidateEntity.id].showName
            ? room.entities[candidateEntity.id].name
            : "",
          ...
          color: room.ffa
            ? room.entities[candidateEntity.id].team == room.clients[key].team
              ? 10
              : room.entities[candidateEntity.id].color
            : room.entities[candidateEntity.id].color,
          ...
          health: room.entities[candidateEntity.id].godMode
            ? 1
            : room.entities[candidateEntity.id].showHealth
            ? room.entities[candidateEntity.id].health /
              room.entities[candidateEntity.id].maxHealth
            : 0,
    ...
    room.clients[key].send(
      protocol.encode(
        "camera",
        room.clients[key].camera.x,
        room.clients[key].camera.y,
        room.clients[key].camera.fov
      )
    );
    ...
    if (room.clients[key].warnings > 50) {
      ...
      room.clients[key].close();
      console.log(
        prefix + "WARNING: Kicked a client for having more than 50 warnings."
      );
...
function broadcastRoom() {
  for (let key in room.clients) {
    ...
    room.clients[key].send(
      protocol.encode("dominationColors", room.dominationColors)
    );
  }
}
...
for (let i = 0; i < 0; i++) {
  ...
  dominator.onDeath = function () {
    ...
    let team =
      room.entities[dominator.lastHurtedEnemy].team == 0 ? "ORANGE" : "RED";
    let otherTeam =
      room.entities[dominator.lastHurtedEnemy].team == 0 ? "RED" : "ORANGE";
    if (dominator.team == -1) {
      dominator.team = room.entities[dominator.lastHurtedEnemy].team;
      dominator.color = room.entities[dominator.lastHurtedEnemy].color;
      broadcastMessage(
        team + " team has claimed the " + domDirection + " dominator"
      );
    } else {
      broadcastMessage(
        team +
          " team is breaking the connection between " +
          otherTeam +
          " team and " +
          domDirection +
          " dominator"
